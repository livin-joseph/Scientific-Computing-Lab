...
def diagdom(coeff_matrix, const_matrix):
    ...
    flag = dd.strictlyDiagonallyDominant(new_coeff_matrix)
    if flag == True:
...
def main():
    ...
    if temp[0] == False:
        print("Given equations cannot be converted into a strictly diagonally dominant matrix")
        import sys
        sys.exit()

    aug_matrix = temp[1]
    print("Given equations are converted into a strictly diagonally dominant matrix")
